...
public class CapturaArticuloNuevo extends AppCompatActivity {
    private ImageView imgvDocumento;
    private String UrlDocumento = "", imgLD, nombreCorto, cveArt, cveSucursal;
    private Button btnEnviar;
    Retrofit retrofit = new RetrofitArticuloNuevo().getRuta();
    PeticionArticuloNuevo peticionArtNuev = retrofit.create(PeticionArticuloNuevo.class);
    ...
    private void GuardaInfo() {
        ...
        Call<RespuestaWSArticuloNuevo> call = peticionArtNuev.subeDocumento(
        ...
        call.enqueue(new Callback<RespuestaWSArticuloNuevo>() {
            @Override
            public void onResponse(@NonNull Call<RespuestaWSArticuloNuevo> call, @NonNull Response<RespuestaWSArticuloNuevo> response) {
                // Obtiene la respuesta del servidor en forma de objeto peticionArtNuev.
                RespuestaWSArticuloNuevo respuesta = response.body();
                // Cancela el ProgressDialog que se muestra durante el envío.
                progressDialog.cancel();
                // Muestra un Toast con el mensaje de respuesta del servidor.
                Toast.makeText(CapturaArticuloNuevo.this, respuesta.getMessage(), Toast.LENGTH_SHORT).show();
            ...
            @Override
            public void onFailure(@NonNull Call<RespuestaWSArticuloNuevo> call, @NonNull Throwable t) {
                // En caso de fallo en la solicitud HTTP, cancela el ProgressDialog y muestra un mensaje de error en un Toast.
                progressDialog.cancel();
                Toast.makeText(CapturaArticuloNuevo.this, "Intermitencia de Internet, Intente Nuevamente", Toast.LENGTH_SHORT).show();
            }
        });
    ...
    private void ActualizaDBs(String url,String id){
        String query = "UPDATE articulo SET urlweb ='"+url+"' WHERE clave = '"+id+"';";
    ...
    class ExampleRunnable implements Runnable {
        ...
        RespuestaWSArticuloNuevo respuesta; // Agrega un campo para almacenar respuesta

        // Constructor de ExampleRunnable que recibe cuatro parámetros: cveSucursal, idReclutamiento, cveArt y respuesta.
        ExampleRunnable(String cveSucursal, String idReclutamiento, String cveArt, RespuestaWSArticuloNuevo respuesta) {
        ...
        public void run() {
            ...
            Call<ResponseDocumentos> call = peticionArtNuev.obtieneListado( cveSucursal);
