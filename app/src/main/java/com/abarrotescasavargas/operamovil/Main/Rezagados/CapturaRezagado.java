...
import com.abarrotescasavargas.operamovil.Main.Rezagado;
...
public class CapturaRezagado extends AppCompatActivity {
    private ImageView imgvDocumento;
    private String UrlDocumento = "", imgLD, nombreCorto, cveArt, cveSucursal;
    private Button btnEnviar;
    Retrofit retrofit = new RetrofitRezagados().getRuta();
    PeticionRezagados peticionRezagados = retrofit.create(PeticionRezagados.class);
    ...
    private void LlenaDatos(ArrayList<Documentos> imgLD, String cveSucursal, String idReclutamiento) {
        Intent intent = new Intent(this, Rezagado.class);
        intent.putExtra("imgLD", imgLD);
        intent.putExtra("nombreCorto", nombreCorto);
    ...
    private void GuardaInfo() {
        ...
        // Crea una llamada (Call) para enviar un documento utilizando la interfaz peticionRezagados.
        Call<RespuestaWSRezagados> call = peticionRezagados.subeDocumento(
                createPartFromString(nombreCorto),
                createPartFromString(cveArt),
        ...
        call.enqueue(new Callback<RespuestaWSRezagados>() {
            @Override
            public void onResponse(@NonNull Call<RespuestaWSRezagados> call, @NonNull Response<RespuestaWSRezagados> response) {
                // Obtiene la respuesta del servidor en forma de objeto RespuestaWSRezagados.
                RespuestaWSRezagados respuesta = response.body();
                // Cancela el ProgressDialog que se muestra durante el envío.
                progressDialog.cancel();
                // Muestra un Toast con el mensaje de respuesta del servidor.
                Toast.makeText(CapturaRezagado.this, respuesta.getMessage(), Toast.LENGTH_SHORT).show();
            ...
            @Override
            public void onFailure(@NonNull Call<RespuestaWSRezagados> call, @NonNull Throwable t) {
                // En caso de fallo en la solicitud HTTP, cancela el ProgressDialog y muestra un mensaje de error en un Toast.
                progressDialog.cancel();
                Toast.makeText(CapturaRezagado.this, "Intermitencia de Internet, Intente Nuevamente", Toast.LENGTH_SHORT).show();
            }
        });
    ...
    private void ActualizaDBs(String url,String id){
        String query = "UPDATE REZAGADOINI SET RI_URLWEB ='"+url+"' WHERE RI_CVEART = '"+id+"';";
    ...
    class ExampleRunnable implements Runnable {
        ...
        RespuestaWSRezagados respuesta; // Agrega un campo para almacenar respuesta

        // Constructor de ExampleRunnable que recibe cuatro parámetros: cveSucursal, idReclutamiento, cveArt y respuesta.
        ExampleRunnable(String cveSucursal, String idReclutamiento, String cveArt, RespuestaWSRezagados respuesta) {
        ...
        public void run() {
            ...
            Call<ResponseDocumentos> call = peticionRezagados.obtieneListado( cveSucursal);
